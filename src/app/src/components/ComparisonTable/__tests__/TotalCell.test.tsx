...
describe('TotalCell', () => {
  test('Displays a formatted value', () => {
    const { queryAllByText } = render(
      <TotalCell cell={{ type: CellType.TOTAL, sizes: { stat: 4300 } }} sizeKey="stat" />
    );
    expect(queryAllByText('4.2 KiB')).toHaveLength(1);
  });

  test('Displays empty string if value is zero', () => {
    const { queryAllByType } = render(<TotalCell cell={{ type: CellType.TOTAL, sizes: { stat: 0 } }} sizeKey="stat" />);
