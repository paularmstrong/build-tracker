...
export default class Queries {
  ...
  getByRevision = async (revision: string): Promise<BuildStruct> => {
    const res = await this.#pool.query('SELECT meta, artifacts FROM builds WHERE revision = $1', [revision]);
    if (res.rowCount !== 1) {
  ...
  getByRevisions = async (revisions: Array<string>): Promise<Array<BuildStruct>> => {
    const res = await this.#pool.query('SELECT meta, artifacts FROM builds WHERE revision = ANY($1)', [revisions]);
    if (res.rowCount === 0) {
      throw new NotFoundError();
    }
  ...
  getRecent = async (limit = 20, branch: string): Promise<Array<BuildStruct>> => {
    ...
    return Promise.resolve(res.rows.sort((a, b) => a.meta.timestamp - b.meta.timestamp));
  };
}
